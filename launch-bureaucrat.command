...
if [[ ! -f "$GMAIL_KEYS" ]]; then
  ...
  printf '   - Under Audience: Click "+ Add users" under Test Users, add your email address, and save.\n\n' >&2
...
if [[ ! -f "$GMAIL_TOKENS" ]]; then
  printf '\n[Gmail]: First-time authorization required. Opening browser...\n'
...
if [[ ! -f "$DRIVE_TOKENS" ]]; then
  printf '\n[Google Drive]: First-time authorization required. Opening browser...\n'
  if ! npx -y @piotr-agier/google-drive-mcp auth; then
